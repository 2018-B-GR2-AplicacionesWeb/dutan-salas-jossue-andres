...
function preguntarIdClienteBuscado(respuestaBDD: RespuestaBDD) {
    return rxjs
        .from(inquirer.prompt(preguntaClienteBusquedaPorId))
        .pipe(
            map( // RESP ANT OBS
...
interface OpcionesCliente {
    opcionesCliente: 'Ingresar Cliente' | 'Buscar Cliente' | 'Borrar Cliente' | 'Actualizar Cliente' | 'Salir';
}
